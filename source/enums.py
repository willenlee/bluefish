...
class ServiceStatus:
    """
    Define the enumeration for the Status property for Rack Manager services.
    ...
    def __init__ (self, value, convert = False):
        """
        Create a Status enumeration instance.
        ...
        if (value not in ServiceStatus.values):
            if (not convert):
                raise ValueError ("{0} is not valid for Status.".format (value))
            ...
            if (value.upper () in {"ENABLED", "RUNNING"}):
                ...
                raise ValueError ("Unknown Status conversion for {0}.".format (value))
